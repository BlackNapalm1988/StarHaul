...
document.addEventListener('DOMContentLoaded', function(){
  ...
  let W=window.innerWidth, H=window.innerHeight; const WORLD={w:10400,h:7800}; const MINI_SIZE=300;
  ...
  function renderDock(){
    ...
    ui.missionList.innerHTML = hereOffers.map(function(m){
      ...
      var illegal=m.illegal?'<span class="badge" style="background:rgba(255,0,0,.15);color:#f99">Illegal</span>':'';
  ...
  function hasIllegalCargo(){ return state.missions.some(function(m){ return m.active && m.illegal; }); }
  function caughtContraband(){ var seized=0; state.missions=state.missions.filter(function(m){ if(m.active && m.illegal){ seized+=m.qty; return false; } return true; }); state.cargo=Math.max(0,state.cargo-seized); state.credits=Math.max(0,state.credits-CFG.patrols.fine); state.reputation=Math.max(0,state.reputation-CFG.contraband.repPenalty); toast('Contraband seized! -$'+CFG.patrols.fine+' • Rep -'+CFG.contraband.repPenalty); updateHUD(); }

  function checkFuelTow(){ if(!state || DEBUG_MODE) return; if(state.fuel>0||state.docked||state.towed) return; var np=nearestPlanet(state.ship.x,state.ship.y); var fee=Math.ceil(CFG.towing.base+CFG.towing.perUnit*np.dist); state.ship.vx=0; state.ship.vy=0; state.credits=Math.max(0,state.credits-fee); state.towed=true; toast('Out of fuel! Towed to '+np.planet.name+' (-$'+fee+')'); var angle=rand(0,Math.PI*2); state.ship.x=np.planet.x+Math.cos(angle)*(np.planet.r+36); state.ship.y=np.planet.y+Math.sin(angle)*(np.planet.r+36); dock(np.planet); updateHUD(); }

  function update(dt){
    var s=state.ship; if(s.centered){ s.x=s.centerX; s.y=s.centerY; s.vx=0; s.vy=0; if(s.thrust) s.centered=false; }
    if(DEBUG_MODE){ state.fuel=1e6; state.credits=1e9; s.hull=s.hullMax; s.lives=99; }
    if(s.flare>0){ s.flare-=1*dt; s.turn=0; s.thrust=false; }
    if(s.cool>0) s.cool--;
    s.a += s.turn*CFG.ship.turn*dt;
    if(s.thrust) s.justSpawned=false;
    if(s.thrust){
      if(DEBUG_MODE){ s.vx += Math.cos(s.a)*CFG.ship.accel*dt; s.vy += Math.sin(s.a)*CFG.ship.accel*dt; }
    ...
    // minefield proximity
    for(var mi=state.mines.length-1; mi>=0; mi--){ var m=state.mines[mi]; if(Math.hypot(s.x-m.x,s.y-m.y)<m.r){ explode(m.x,m.y,24); damage(CFG.mines.damage); state.mines.splice(mi,1); } }
    ...
    // meteor showers
    state.nextMeteor-=1*dt;
    if(state.nextMeteor<=0){ state.meteorEvents.push(startMeteorEvent()); state.nextMeteor=irand(CFG.meteors.spawnEvery[0],CFG.meteors.spawnEvery[1]); toast('☄️ Meteor shower!'); }
    for(var me=state.meteorEvents.length-1; me>=0; me--){ var ev=state.meteorEvents[me]; ev.t-=1*dt; ev.spawn-=1*dt; if(ev.spawn<=0){ ev.spawn=60/CFG.meteors.rate; var ex=ev.x+Math.cos(ev.angle)*rand(-220,220); var ey=ev.y+Math.sin(ev.angle)*rand(-220,220); state.asteroids.push(makeAsteroid(ex,ey,CFG.asteroid.sizes[2])); } if(ev.t<=0){ state.meteorEvents.splice(me,1); } }
    ...
    state.bases.forEach(function(b){ b.spawn-=1*dt; b.cool-=1*dt; if(b.spawn<=0){ var np=makePirate(); np.x=b.x+rand(-60,60); np.y=b.y+rand(-60,60); state.pirates.push(np); b.spawn=CFG.pirateBase.spawnEvery; } if(b.cool<=0){ var dx=state.ship.x-b.x, dy=state.ship.y-b.y; var d=Math.hypot(dx,dy); if(d<420){ var a=Math.atan2(dy,dx); state.bullets.push({x:b.x,y:b.y,vx:Math.cos(a)*3,vy:Math.sin(a)*3,life:220,r:3,enemy:true}); b.cool=CFG.pirateBase.fireEvery; } } });

    state.hunters.forEach(function(h){ var dx=state.ship.x-h.x, dy=state.ship.y-h.y; var d=Math.hypot(dx,dy)||1; h.vx+=(dx/d)*0.05*dt; h.vy+=(dy/d)*0.05*dt; var sp=Math.hypot(h.vx,h.vy); if(sp>CFG.hunters.speed){ h.vx=(h.vx/sp)*CFG.hunters.speed; h.vy=(h.vy/sp)*CFG.hunters.speed; } h.x+=h.vx*dt; h.y+=h.vy*dt; wrapEntity(h); h.cool-=1*dt; if(h.cool<=0){ var lead=d/CFG.hunters.bulletSpeed; var tx=state.ship.x+state.ship.vx*lead; var ty=state.ship.y+state.ship.vy*lead; var ang=Math.atan2(ty-h.y,tx-h.x); state.bullets.push({x:h.x,y:h.y,vx:Math.cos(ang)*CFG.hunters.bulletSpeed,vy:Math.sin(ang)*CFG.hunters.bulletSpeed,life:220,r:3,enemy:true}); h.cool=CFG.hunters.fireEvery; } });

    state.patrols.forEach(function(p){ p.x+=p.vx*dt; p.y+=p.vy*dt; wrapEntity(p); p.a=Math.atan2(p.vy,p.vx); p.scan-=1*dt; if(p.scan<=0){ p.scan=120; var dx=state.ship.x-p.x, dy=state.ship.y-p.y; var d=Math.hypot(dx,dy); var ang=Math.atan2(dy,dx); var diff=Math.abs(Math.atan2(Math.sin(ang-p.a),Math.cos(ang-p.a))); if(d<CFG.patrols.scanRadius && diff<CFG.patrols.cone){ if(hasIllegalCargo()) caughtContraband(); } } });
    ...
    for(var si=0; si<state.stars.length; si++){ var st=state.stars[si]; if(!DEBUG_MODE && Math.hypot(state.ship.x-st.x,state.ship.y-st.y) < (state.ship.r*0.9 + st.r)){ return gameOver(); } }
    ...
    for(var bi=state.bases.length-1; bi>=0; bi--){ var B=state.bases[bi]; for(var bjb=state.bullets.length-1; bjb>=0; bjb--){ var bl=state.bullets[bjb]; if(bl.enemy) continue; if(Math.hypot(B.x-bl.x,B.y-bl.y)<B.r){ state.bullets.splice(bjb,1); B.hp-=10; explode(B.x,B.y,12); if(B.hp<=0){ state.bases.splice(bi,1); state.credits+=CFG.pirateBase.bounty; state.spawnTimer+=600; toast('Pirate base destroyed! +$'+CFG.pirateBase.bounty); updateHUD(); } break; } } if(state.ship.inv<=0 && Math.hypot(B.x-state.ship.x,B.y-state.ship.y)<B.r+state.ship.r*.8){ damage(20); } }

    for(var hu=state.hunters.length-1; hu>=0; hu--){ var H=state.hunters[hu]; for(var bjh=state.bullets.length-1; bjh>=0; bjh--){ var bb2=state.bullets[bjh]; if(bb2.enemy) continue; if(Math.hypot(H.x-bb2.x,H.y-bb2.y)<H.r+3){ state.bullets.splice(bjh,1); explode(H.x,H.y,8); state.hunters.splice(hu,1); state.credits+=80; updateHUD(); break; } } if(state.ship.inv<=0 && Math.hypot(H.x-state.ship.x,H.y-state.ship.y)<H.r+state.ship.r*.8){ damage(15); } }
  ...
  function draw(){
    ...
    ctx.globalAlpha=1; var bgNeb=ctx.createRadialGradient(W/2,H/2,0,W/2,H/2,900); bgNeb.addColorStop(0,'rgba(109,184,242,0.08)'); bgNeb.addColorStop(1,'rgba(0,0,0,0)'); ctx.fillStyle=bgNeb; ctx.fillRect(0,0,W,H);
    ...
    // mines
    state.mines.forEach(function(m){ var b=(Math.sin(frame/20 + m.blink)+1)/2; ctx.fillStyle='rgba(255,180,180,'+(0.1+0.5*b)+')'; ctx.beginPath(); ctx.arc(m.x,m.y,2,0,Math.PI*2); ctx.fill(); });
    ...
    // pirate bases
    state.bases.forEach(function(b){ ctx.save(); ctx.translate(b.x,b.y); ctx.fillStyle='#633'; ctx.beginPath(); ctx.arc(0,0,b.r,0,Math.PI*2); ctx.fill(); ctx.strokeStyle='#a33'; ctx.lineWidth=2; ctx.beginPath(); ctx.arc(0,0,b.r,0,Math.PI*2); ctx.stroke(); ctx.restore(); });

    // patrols
    state.patrols.forEach(function(p){ ctx.save(); ctx.translate(p.x,p.y); ctx.rotate(p.a); ctx.strokeStyle='#4f8'; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(p.r,0); ctx.lineTo(-p.r*0.6,-p.r*0.6); ctx.lineTo(-p.r*0.6,p.r*0.6); ctx.closePath(); ctx.stroke(); ctx.restore(); });
    ...
    // bounty hunters
    state.hunters.forEach(function(h){ ctx.save(); ctx.translate(h.x,h.y); ctx.rotate(Math.atan2(state.ship.y-h.y,state.ship.x-h.x)); ctx.strokeStyle='#fff'; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(h.r,0); ctx.lineTo(-h.r*.6,-h.r*.6); ctx.lineTo(-h.r*.3,0); ctx.lineTo(-h.r*.6,h.r*.6); ctx.closePath(); ctx.stroke(); ctx.restore(); });
    ...
    if(state.inRadiation){ ctx.save(); ctx.fillStyle='rgba(0,255,0,0.12)'; ctx.fillRect(0,0,W,H); ctx.restore(); }
  ...
  function loop(ts){ if(!running) return; if(!lastTime){ lastTime=ts; } var dt=(ts-lastTime)/(1000/60); if(dt<=0||dt>5) dt=1; lastTime=ts; if(!paused && !state.gameOver){ update(dt); } if(state) draw(); frame++; rAF(loop); }
  ...
  function showMenu(){ running=false; paused=false; showGameUI(false); ui.dockUI.style.display='none'; ui.missionLog.style.display='none'; ui.startScreen.classList.remove('hidden'); ui.leaderboard.classList.add('hidden'); ui.gameOver.classList.add('hidden'); ui.pauseOverlay.classList.add('hidden'); ui.settingsScreen.classList.add('hidden'); }
  function startNewGame(){ DEBUG_MODE = ui.debugToggle && ui.debugToggle.checked; reset(); if(DEBUG_MODE){ state.credits=1e9; state.fuel=1e6; state.ship.hull=state.ship.hullMax; state.ship.lives=99; updateHUD(); } lastTime=0; running=true; paused=true; ui.startScreen.classList.add('hidden'); ui.settingsScreen.classList.add('hidden'); ui.gameOver.classList.add('hidden'); showGameUI(true); rAF(loop); }
  ...
  ui.settingsBtn.addEventListener('click', function(){ ui.startScreen.classList.add('hidden'); ui.settingsScreen.classList.remove('hidden'); });
  ui.settingsBackBtn.addEventListener('click', function(){ ui.settingsScreen.classList.add('hidden'); ui.startScreen.classList.remove('hidden'); });
  ...
  function assert(cond, name){ if(!cond){ console.error('Test failed:', name); toast('Self-test failed: '+name); throw new Error('Self-test failed: '+name); } else { console.log('✓', name); } }
  function runSelfTests(){
    console.log('Running StarHaul self-tests…');
    ...
    // pirate collides with asteroid → takes damage or dies
